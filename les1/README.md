...
> [!NOTE]
> **Recent Updates (BPE Tokenizer & Movie Titles Dataset):**
> We have extended this educational repository to support **Byte Pair Encoding (BPE)** tokenization trained on the Hugging Face [Pablinho/movies-dataset](https://huggingface.co/datasets/Pablinho/movies-dataset):
> - **Tokenizer**: A byte-level BPE tokenizer trained on ~9,500 unique movie titles (saved to [`data/bpe_tokenizer.json`](data/bpe_tokenizer.json) via [`train_tokenizer.py`](train_tokenizer.py)).
> - **Data**: Automatic download and preprocessing pipeline (clearing case and whitespace, deduplicating) via [`data/download_data.py`](data/download_data.py).
> - **Model Updates**: The `qwen3_5` (hybrid attention) directory was updated to train on movie titles using the new BPE tokenizer.
> - **Notebook**: [`demo.ipynb`](demo.ipynb) has been configured to use the Qwen3.5 architecture and the BPE Tokenizer seamlessly.
...
Four language-model architectures, same task, side by side:
